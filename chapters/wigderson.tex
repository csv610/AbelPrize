\chapter{Avi Wigderson (2021)}\index{Wigderson, Avi}
...
Avi Wigderson\index{Wigderson, Avi} is one of the founding figures of computational complexity theory\index{Complexity theory}. His work on interactive proofs, zero-knowledge protocols, and the role of randomness in computation has fundamentally shaped our understanding of what can be efficiently computed and verified.
...
Now imagine you want to convince someone that you know a secret without revealing the secret itself. Wigderson, together with Goldreich and Micali, proved that every NP problem has a \emph{zero-knowledge proof}\index{Zero-knowledge proofs}---a protocol that conveys the truth of a statement and nothing else.
...
\item \textbf{P vs. NP}\index{P vs NP}: The hardness-vs-randomness paradigm connects the central open problem to the power of randomness.
\end{itemize}
...
\item \textbf{The Zig-Zag Product}: With Reingold and Vadhan: a new operation on graphs that constructs expanders\index{Expander graphs} with optimal parameters. The zig-zag product is a graph-theoretic analog of the semidirect product in group theory.
