...
\begin{knowledgebridge}{What Does Homotopy Equivalent Mean?}
Two spaces are \textbf{homotopy equivalent} if one can be continuously deformed into the other, allowing stretching and shrinking but not tearing. For example, a solid disk is homotopy equivalent to a point (you can shrink it). The $n$-sphere $S^n$ is not homotopy equivalent to a point because there is a ``hole'' that cannot be filled. Milnor's spheres were homotopy equivalent to $S^7$ but not diffeomorphic to it.
\end{knowledgebridge}
...
\begin{primer}{The Signature Theorem}
The \textbf{signature} of a $4k$-dimensional manifold is a topological invariant: the number of positive eigenvalues minus negative eigenvalues of the intersection form on middle cohomology. Hirzebruch's theorem expresses it as a polynomial in the Pontryagin classes (smooth invariants). Milnor used this bridge between topology and smooth geometry to detect exotic spheres.
\end{primer}
...
\centering
\begin{tikzpicture}[
    ball/.style={circle, draw, thick, minimum size=2.2cm, align=center, font=\small},
    arr/.style={->, thick, abelred, bend left=20}
]

% Left disk: D^4_+ × S^3
\node[ball, fill=abelblue!10, draw=abelblue] (dplus) at (-3, 0) {$D^4_+ \times S^3$};
\node[font=\footnotesize, abelblue, below=2pt] at (dplus.south) {$\partial = S^3 \times S^3$};

% Right disk: D^4_- × S^3
\node[ball, fill=abelgreen!10, draw=abelgreen] (dminus) at (3, 0) {$D^4_- \times S^3$};
\node[font=\footnotesize, abelgreen, below=2pt] at (dminus.south) {$\partial = S^3 \times S^3$};

% Gluing map arrow
\draw[arr, abelred] (dplus.east) to node[midway, above, font=\small, abelred] {$f_{k,l}: S^3 \to SO(4)$} (dminus.west);

% Result label
\node[draw, thick, rounded corners, fill=abelfill, minimum width=7cm, minimum height=0.8cm] (result) at (0, -3) {$M_{k,l} = D^4_+ \times S^3 \;\cup_{f_{k,l}}\; D^4_- \times S^3$};

\draw[->, thick, abelgray] (0, -1.2) -- (result.north);

% Note
\node[font=\footnotesize\itshape, align=center] at (0, -4.2) {Homeomorphic to $S^7$ but equipped with a different smooth structure.\\[2pt]
The pair $(k,l) \in \mathbb{Z} \times \mathbb{Z}$ classifies the $S^3$-bundle via $\pi_3(SO(4)) \cong \mathbb{Z} \oplus \mathbb{Z}$.};

% Schematic boundaries as circles
\draw[thick, abelblue] (-3, 0) circle (1.1cm);
\draw[thick, abelgreen] (3, 0) circle (1.1cm);

% S^3 indicator
\node[font=\footnotesize] at (-3, 1.8) {$S^3$ (fiber)};
\node[font=\footnotesize] at (3, 1.8) {$S^3$ (fiber)};

\end{tikzpicture}
\caption{Milnor's construction of exotic 7-spheres: two copies of $D^4 \times S^3$ are glued along their common boundary $S^3 \times S^3$ by a map $f_{k,l}: S^3 \to SO(4)$ representing an element of $\pi_3(SO(4)) \cong \mathbb{Z} \oplus \mathbb{Z}$. The resulting manifold $M_{k,l}$ is homeomorphic to $S^7$ but for certain $(k,l)$ carries a distinct smooth structure, yielding 28 exotic spheres in dimension 7.}
\label{fig:exotic_spheres}
\end{figure}
...
\begin{bigidea}
Milnor's revolutionary insight was to use the \textbf{Hirzebruch signature theorem} as a detector of smooth structure. The signature is a topological invariant, but the Pontryagin classes are smooth invariants. By forcing the signature formula to yield an integer, Milnor obtained a Diophantine condition $(k-l)^2 \equiv 1 \pmod{45}$ that selects exactly the dimensions where exotic spheres exist.
\end{bigidea}
...
% Height axis
\draw[thick, abelblue, ->] (0,0) -- (0,4.5) node[above] {$f$};

% Critical values on axis
\foreach \y/\label in {0.5/$c_0$, 2.0/$c_1$, 3.5/$c_2$} {
  \draw[thick, abelblue] (-0.1,\y) -- (0.1,\y) node[left] {\label};
  \draw[dashed, abelgray] (0,\y) -- (5.5,\y);
}

% Surface (mountain landscape)
\draw[thick, fill=abelfill, smooth] 
  plot[domain=0:5.5, samples=100] (\x, {0.8 + 2.0*exp(-(\x-2.5)^2/1.5) + 0.6*sin(deg(\x)*1.8)});

% Critical points
\fill[abelred] (1.0, 1.0) circle (3pt) node[below, font=\footnotesize] {minimum (index 0)};
\fill[abelred] (2.8, 2.6) circle (3pt) node[above right, font=\footnotesize] {saddle (index 1)};
\fill[abelred] (4.8, 3.2) circle (3pt) node[above, font=\footnotesize] {maximum (index 2)};

% Level set shading: sublevel sets f^{-1}(-∞, c]
\fill[abelblue, opacity=0.15] (0,0) -- plot[domain=0:5.5, samples=100] (\x, {0.8 + 2.0*exp(-(\x-2.5)^2/1.5) + 0.6*sin(deg(\x)*1.8)}) -- (5.5,0) -- cycle;
\fill[abelred, opacity=0.10] (0,0) -- plot[domain=0:5.5, samples=100] (\x, {0.8 + 2.0*exp(-(\x-2.5)^2/1.5) + 0.6*sin(deg(\x)*1.8)}) -- (5.5,0) -- cycle;

% Level set lines
\draw[thick, abelgreen, dashed] plot[domain=0:5.5, samples=100] (\x, {0.8 + 2.0*exp(-(\x-2.5)^2/1.5) + 0.6*sin(deg(\x)*1.8)});

% Handle attachment visualization (schematic)
\node[draw, rounded corners, fill=abelblue!10, abelblue, thick, minimum width=2cm, minimum height=0.5cm] (h0) at (7, 1.0) {0-handle $D^0$};
\node[draw, rounded corners, fill=abelred!10, abelred, thick, minimum width=2cm, minimum height=0.5cm] (h1) at (7, 2.6) {1-handle $D^1$};
\node[draw, rounded corners, fill=abelgreen!10, abelgreen, thick, minimum width=2cm, minimum height=0.5cm] (h2) at (7, 3.2) {2-handle $D^2$};

\draw[->, thick, abelgray, dashed] (1.0, 1.0) -- (h0.west);
\draw[->, thick, abelgray, dashed] (2.8, 2.6) -- (h1.west);
\draw[->, thick, abelgray, dashed] (4.8, 3.2) -- (h2.west);

% Bottom label
\node[font=\small, align=center] at (3, -0.5) {Schematic Morse landscape: sublevel sets $f^{-1}(-\infty, c]$\\ fill as $c$ increases; a $k$-handle attaches at each critical point of index $k$.};

\end{tikzpicture}
\caption{Morse theory on a smooth manifold: a height function $f$ with three critical points (minimum, saddle, maximum) of indices 0, 1, and 2. As the sublevel set $f^{-1}(-\infty, c]$ passes a critical value, a handle of the corresponding index is attached, building the manifold cell by cell.}
...
\begin{analogy}{Characteristic Classes as Shadows}
Characteristic classes are like \textbf{shadows} cast by a vector bundle onto the base space. Just as a shadow reveals the shape of an object without showing all its details, a characteristic class reveals essential twisting information about a bundle. Stiefel--Whitney, Chern, and Pontryagin classes each capture different aspects of this twisting.
\end{analogy}
...
\begin{whyitmatters}
The discovery of exotic spheres \textbf{created the field of differential topology}. It showed that smoothness is not a passive property of a space but carries genuine information. This insight underpins modern manifold classification via surgery theory, the study of exotic $\mathbb{R}^4$, and even string theory compactifications where distinct smooth structures correspond to distinct physical vacua.
\end{whyitmatters}
\end{itemize}
...
\subsection{Exotic \texorpdfstring{$\mathbb{R}^4$}{R\^{}4}}
...
\subsection{Algebraic \texorpdfstring{$K$}{K}-Theory and Motivic Cohomology}
...
\subsection{The \texorpdfstring{$h$}{h}-Cobordism Theorem and 4-Manifolds}
...
\subsection{The \texorpdfstring{$\hat{A}$}{A}-Genus and Exotic Spheres}
...
\subsection{Topological Hochschild Homology and \texorpdfstring{$K$}{K}-Theory}
...
The problems Milnor opened---the classification of smooth structures, the relationship between algebraic $K$-theory and Galois cohomology, the dynamics of rational maps---remain active research areas. His work provides both the foundations and the inspiration for much of modern geometry and topology. The Abel Prize citation recognized Milnor ``for pioneering discoveries in topology, geometry and algebra.''
