...
\begin{knowledgebridge}{What Is an Abelian Extension?}
A field extension $L/K$ is \textbf{abelian} if its Galois group $\mathrm{Gal}(L/K)$ is an abelian group (the group operation commutes). Class field theory gives a complete dictionary: the abelian extensions of a number field $K$ correspond exactly to certain groups of ideals. It is one of the greatest achievements of early 20th-century number theory.
\end{knowledgebridge}
...
\begin{primer}{What Is Group Cohomology?}
Group cohomology measures \textbf{obstructions} to finding invariant solutions under a group action. For $G$ acting on $A$:
\begin{itemize}
\item $H^0(G,A)$: elements fixed by $G$.
\item $H^1(G,A)$: crossed homomorphisms modulo principal ones.
\item $H^2(G,A)$: group extensions.
\end{itemize}
Tate extended these to negative indices, creating a unified periodic theory.
\end{primer}
...
\begin{knowledgebridge}{The Langlands Program}
The Langlands program is a vast web of conjectures connecting number theory to representation theory. It predicts that every $L$-function from number theory (like those Tate studied) corresponds to an \textbf{automorphic form}---a highly symmetric function on a Lie group. Tate's adelic treatment of Hecke $L$-functions provided the analytic template for this entire program.
\end{knowledgebridge}
...
\begin{bigidea}
Tate's key insight: the Artin reciprocity law is equivalent to the \textbf{periodicity} of Tate cohomology groups. The Tate--Nakayama theorem gives an isomorphism $\widehat H^q(G,\mathbb{Z}) \cong \widehat H^{q+2}(G,C_L)$, which encodes the entire reciprocity law as a purely algebraic statement. This replaced hundreds of pages of analytic number theory with clean algebra.
\end{bigidea}
...
\begin{whyitmatters}
The Tate--Shafarevich group $\mathrm{III}(E/K)$ is the \textbf{gatekeeper} of the Birch and Swinnerton-Dyer conjecture. It measures when local solvability everywhere fails to guarantee a global rational point. Proving $\mathrm{III}$ is finite---and computing its size from the $L$-function---would earn you \$1 million (one of the Clay Millennium Problems).
\end{whyitmatters}
...
\begin{analogy}{The Tate Module as a Microscope}
Just as a microscope reveals finer details at higher magnification, the $\ell$-adic Tate module $T_\ell(E) = \varprojlim E[\ell^n]$ lets us study an elliptic curve at \textbf{arbitrarily fine scales}. Each level $E[\ell^n]$ shows the $\ell^n$-torsion points, and the inverse limit captures all of them at once. The Galois group acts on this structure, turning geometry into representation theory.
\end{analogy}
...
\subsection{\texorpdfstring{$p$}{p}-adic Hodge Theory}\index{Hodge theory}
...
John Tate's work transformed number theory and arithmetic geometry. His thesis, cohomology, and conjectures are now fundamental parts of the subject. The Abel Prize citation recognized Tate ``for his vast and lasting impact on the theory of numbers.''
