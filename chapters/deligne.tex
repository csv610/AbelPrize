...
\begin{knowledgebridge}{Counting Over Finite Fields}
When mathematicians say they are counting solutions to an equation like $y^2 = x^3 - x$ ``modulo $p$,'' they mean they are looking for remainders after division by $p$. For each prime $p$, the number $N_p$ counts how many pairs $(x,y)$ with $0 \leq x,y < p$ satisfy the equation. The Weil conjectures assert that the sequence $\{N_p\}$ is not random at all---it is governed by the topology of the curve as a geometric object over the complex numbers.
\end{knowledgebridge}
...
\begin{primer}{What is a Zeta Function?}
A zeta function is a mathematical tool that encodes information about a set of objects (like prime numbers or solutions to equations) into a single function. Riemann's zeta function $\zeta(s) = \sum_{n=1}^\infty n^{-s}$ packs information about all prime numbers into one analytic function. The Riemann hypothesis---stating that all nontrivial zeros of $\zeta(s)$ lie on the line $\Re(s)=1/2$---would reveal deep regularities in how primes are distributed. Weil's analogue for curves over finite fields plays the same role for counting points on those curves.
\end{primer}
...
\centering
\begin{tikzpicture}[
    greenbox/.style={draw=abelgreen, fill=green!8, rounded corners,
                     minimum width=5.2cm, minimum height=2.2cm,
                     text centered, font=\sffamily},
    bluebox/.style={draw=abelblue, fill=blue!5, rounded corners,
                    minimum width=5.2cm, minimum height=2.2cm,
                    text centered, font=\sffamily},
]
\node[greenbox, text width=4.8cm, align=center] (rat) at (-3, 1.8)
    {\textbf{Rationality}\\ $Z(X,t)\in\mathbb{Q}(t)$\\ \small\textit{Proved by Grothendieck}};
\node[greenbox, text width=4.8cm, align=center] (fe) at ( 3, 1.8)
    {\textbf{Functional Equation}\\ $Z(X,\frac{1}{q^n t})=\pm q^{n\chi/2}t^\chi Z(X,t)$\\ \small\textit{Proved by Grothendieck}};
\node[bluebox, text width=4.8cm, align=center] (rh) at (-3,-1.8)
    {\textbf{Riemann Hypothesis}\\ Zeros lie on $|t|=q^{-k/2}$\\ \small\textit{Proved by Deligne}};
\node[bluebox, text width=4.8cm, align=center] (bn) at ( 3,-1.8)
    {\textbf{Betti Numbers}\\ $\deg P_i = \dim H^i(X(\mathbb{C}),\mathbb{Q})$\\ \small\textit{Proved by Deligne}};

\node[text width=10cm, align=center, font=\small, yshift=-3.2cm]
    at (0,0)
    {\textbf{Consequence:} The number of $\mathbb{F}_q$-points on a
     smooth projective variety is determined by its topological
     Betti numbers, revealing a deep link between arithmetic and
     geometry.};
\end{tikzpicture}
\caption{The four Weil conjectures, color-coded by prover. Grothendieck's $\ell$-adic cohomology proved (1) and (2); Deligne proved (3) and (4).}
\label{fig:weil-conjectures}
\end{figure}
...
\subsection{\texorpdfstring{$\ell$}{l}-adic Cohomology}\index{Cohomology}
...
\begin{bigidea}
Cohomology is the art of turning geometric shapes into algebraic data. Instead of tracking every point on a variety $X$, cohomology groups $H^i(X)$ summarize its ``holes'' of different dimensions, just as the holes in a donut or a pretzel characterize its shape. The Frobenius map $F$ acts on these groups, and the Lefschetz trace formula says that counting fixed points of $F$---which is exactly $N_m$---reduces to computing traces of linear operators. Geometry becomes linear algebra.
\end{bigidea}
...
\begin{analogy}{The Induction Staircase}
Deligne's inductive proof is like building a tall building floor by floor. The base case (curves) is the ground floor---already built by Weil. To reach the next floor ($n$-dimensional varieties), he uses a Lefschetz pencil: a family of hyperplane sections that act like cross-sections of the building. The hard Lefschetz theorem ensures the cross-sections capture the whole structure, and the Hodge--Riemann relations provide the ``stress test'' that the eigenvalues cannot exceed their allowed size.
\end{analogy}
...
\begin{whyitmatters}
Before Deligne, number theory and geometry were separate fields that occasionally exchanged ideas. After Deligne, they became a single subject: arithmetic geometry. Every modern result in the Langlands program, every application of $\ell$-adic cohomology to cryptography, and every use of mixed Hodge structures in algebraic geometry traces back to Deligne's proof of the Weil conjectures. His work is the foundation on which much of twenty-first-century number theory and algebraic geometry is built.
\end{whyitmatters}
...
\subsection{Perfectoid Spaces and \texorpdfstring{$p$}{p}-adic Geometry}\index{Perfectoid spaces}
